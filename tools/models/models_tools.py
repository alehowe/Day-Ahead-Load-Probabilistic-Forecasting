...
from tools.models.CNNMIXED import CNNMIXEDRegressor
from tools.models.MIXED import MIXEDRegressor
from tools.models.RNN import RNNRegressor
...
def get_model_class_from_conf(conf):
    ...
    if conf == 'ARX':
        ...
        model_class = CNNRNNRegressor
    elif conf == 'MIXED':
        model_class = MIXEDRegressor

    elif conf == 'CNNMIXED':
        model_class = CNNMIXEDRegressor
    else:
...
class TensorflowRegressor():
    ...
    def __init__(self, settings, sample_x, ):
        ...
        if settings['model_class'] == 'DNN':
        ...
        elif settings['model_class'] == 'RNN':
            model_input = RNNRegressor.build_model_input_from_series(
                x=sample_x,
                col_names=self.x_columns_names,
                pred_horiz=self.pred_horiz
            )
            settings['n_time_steps'] = model_input.shape[1]
            settings['n_features'] = model_input.shape[2]
            # number of time steps
            # Build the model architecture
            self.regressor = RNNRegressor(settings, loss)
        elif settings['model_class'] == 'CNNRNN':
            print("---------------------------------------------------------------")
            model_input = CNNRNNRegressor.build_model_input_from_series(
                x=sample_x,
                col_names=self.x_columns_names,
                pred_horiz=self.pred_horiz
            )
            ...
            # number of time steps
            # Build the model architecture
            self.regressor = CNNRNNRegressor(settings, loss)

        elif settings['model_class'] == 'MIXED':
            # get input size for the chosen model architecture
            input1, input2 = MIXEDRegressor.build_model_input_from_series(x=sample_x,
                                                                          col_names=self.x_columns_names,
                                                                          pred_horiz=self.pred_horiz)
            settings['dense_input_size'] = input2.shape[1]

            settings['n_time_steps'] = input1.shape[1]

            settings['n_features'] = input1.shape[2]

            self.regressor = MIXEDRegressor(settings, loss)

        elif settings['model_class'] == 'CNNMIXED':
            # get input size for the chosen model architecture
            input1, input2 = CNNMIXEDRegressor.build_model_input_from_series(x=sample_x,
                                                                          col_names=self.x_columns_names,
                                                                          pred_horiz=self.pred_horiz)
            settings['dense_input_size'] = input2.shape[1]
            ...
            self.regressor = CNNMIXEDRegressor(settings, loss)
    ...
    def fit(self, train_x, train_y, val_x, val_y, verbose=0, pruning_call=None, plot_history=False):
        history = self.regressor.fit(train_x, train_y, val_x, val_y, verbose=1, pruning_call=None)
        if plot_history:
    ...
    def __pred_JSU_params__(self, pred_dists: tfp.distributions):
        ...
        # Expand dimension to enable concat in ensemble
        return tf.expand_dims(tf.concat([loc, scale, skewness, tailweight], axis=-1), axis=2)
...
class Ensemble():
    ...
    def __get_qr_PIs__(preds_test, settings):
        # simply flatten in temporal dimension: 2d matriz
    ...
    def __build_JSU_PIs__(preds_test, settings):
        ...
        for k in range(preds_test.shape[2]):
            pred_samples.append(tfd.JohnsonSU(
                ...
                skewness=preds_test[:, :, k, 2],
                tailweight=preds_test[:, :, k, 3]).sample(10000).numpy())
        return np.transpose(np.quantile(np.concatenate(pred_samples, axis=0),
                                        q=settings['target_quantiles'], axis=0),
                            axes=(1, 2, 0)).reshape(-1, len(settings['target_quantiles']))
