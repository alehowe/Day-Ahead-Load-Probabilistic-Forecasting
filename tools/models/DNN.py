...
class DNNRegressor:
    ...
    def __build_model__(self, loss):
        ...
        x = (tf.keras.layers.Dense(self.settings['hidden_size'],
                                  activation=self.snake, #activation=self.settings['activation'],
                                  )(x_in))
        ...
        if self.settings['PF_method'] == 'point':
            out_size = 1
            # As many neurons as the number of vars to predicts for the day ahead
            ...
            output = tfp.layers.DistributionLambda(lambda t: tfd.JohnsonSU(
                ...
                scale=1e-3 + 3 * tf.math.softplus(0.05 * t[..., self.settings['pred_horiz']:2*self.settings['pred_horiz']]),
                skewness= t[..., 2*self.settings['pred_horiz']:3*self.settings['pred_horiz']],
                tailweight= 1+3*tf.math.softplus(t[..., 3*self.settings['pred_horiz']:4*self.settings['pred_horiz']])
            ))(logit)
    ...
    def build_model_input_from_series(x, col_names: List, pred_horiz: int):
        ...
        # get indexes of the moddified PC variables
        pc_col_idxs = [index for (index, item) in enumerate(col_names)
                          if features_keys['pc'] in item]

        # get indexes of the moddified PC variables
        dummy_col_idxs = [index for (index, item) in enumerate(col_names)
                       if features_keys['dummy'] in item]
        ...
        # build conditioning variables for transformed features
        pc_feat = [x[:, -pred_horiz:, feat_idx] for feat_idx in pc_col_idxs]

        # build conditioning variables for dummy features
        dummy_feat = [x[:, -pred_horiz:, feat_idx] for feat_idx in dummy_col_idxs]

        # return flattened input
        return np.concatenate(past_feat + futu_feat + c_feat + pc_feat + dummy_feat, axis=1)
